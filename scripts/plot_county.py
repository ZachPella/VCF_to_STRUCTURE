...
# File names
input_file = f"K{K}.indfile"
# 🚨 UPDATED LABEL FILE NAME
label_file = "name_and_state_county.txt"
output_pdf = f"Admixture_Labeled_K{K}_COUNTY_IOWA_NE_KS.pdf" # Updated output name

# --- 1. Read Q-matrix from .indfile ---
...
try:
    with open(input_file, 'r') as f:
        ...
        def parse_line(line, K):
            """Parses a single line to extract K Q-values."""
            q_match = re.search(r':\s+((?:\d+\.\d+\s*){' + str(K) + r'})', line)
            if q_match:
                proportions_str = q_match.group(1).strip()
                proportions = [float(x) for x in proportions_str.split()]
                if len(proportions) == K:
                    return proportions
            return None

        first_line = f.readline()
        first_proportions = parse_line(first_line, K)
        if first_proportions:
            q_data.append(first_proportions)
        else:
            raise Exception("Failed to parse the first line of the .indfile.")
        ...
        for line in f:
            ...
            proportions = parse_line(line, K)
            if proportions:
                q_data.append(proportions)

    if not q_data:
        raise Exception("Failed to extract any Q-matrix data.")
...
# ----------------------------------------------------------------------
# --- 2.5 Grouping and Sorting Data (UPDATED GROUP_ORDER) ---
# ----------------------------------------------------------------------
print("Grouping and sorting samples by County/State...")

# 🚨 GROUP_ORDER MODIFIED: Iowa first, then Nebraska counties in specified order, then Kansas.
GROUP_ORDER = [
    'Iowa',
    'Nebraska_Thurston',
    'Nebraska_Dodge',
...
# Sort by the sort_key (group order), then by the original label alphabetically within groups
...
fig, ax = plt.subplots(figsize=(max(8, len(labels) * 0.15), 6))
